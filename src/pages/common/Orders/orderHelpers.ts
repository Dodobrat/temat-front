...
export const parsedFetchedData = (order) => {
	const parsedOrderData: any = { orderId: null, payment: {}, shipping: {}, receiver: {}, products: [], extras: {} };
	...
	parsedOrderData.receiver.receiverPhoneCodeId = order?.client?.receiverPhoneCode;
	...
	parsedOrderData.receiver.receiverAgentPhoneCodeId = order?.client?.receiverAgentPhoneCode;

	//Products
	const productsWithIds = [];
	for (const product of order?.products) {
		productsWithIds.push({ ...product, value: product?.productId, quantity: product?.required, price: product?.price });
	}
